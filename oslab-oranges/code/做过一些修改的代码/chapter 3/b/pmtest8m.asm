...
Checkadd:
	...
	mov	eax, LinearAddrDemo ;取线性地址高10位，计算PDE在页目录表中位置
	shr	eax, 22
	mov ecx,eax
	shl ecx, 2				;一项4字节,故*4
	add ecx,PageDirBase1
	mov eax, [ss:ecx]  		; eax保存对应PDE的32位数据
	mov ebx,eax
	push ecx
	call DispInt  ;会改变eax的值,所以增加ebx，保存eax
	pop ecx
	
	shr ebx,12				;PDE32位中高20位为地址，清除低12位
	shl ebx,12				;得页表基址
	push ebx
	call DispInt
	pop ebx
	...
	shl eax,10
	shr	eax,22
	mov ecx,eax		;取线性地址中间十位，计算PTE在页表中位置
	shl ecx,2		;一项4字节,故*4
	add ebx ,ecx	;ebx为页表基址,+ecx得PTE地址
	mov eax, [ss:ebx]	; 	取PTE。之后取高20位，即为页基址
	shr eax,12
	shl eax,12
	push eax
	call DispInt
