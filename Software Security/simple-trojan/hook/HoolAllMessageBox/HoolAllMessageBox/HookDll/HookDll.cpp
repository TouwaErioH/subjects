...
TypeGetDesk oldGetDesk = NULL;	// 用于保存原楷书地址
FARPROC pfGetDesk = NULL;		// 指向原函数地址的远指针
...
/*aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa*/
BYTE oldCodeG[CODE_LENGTH];	// 保存原来API入口代码
BYTE newCodeG[CODE_LENGTH];	// 保存新API入口代码，jmp xxxx
...
void HookOn()
{
	...
	/*aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa*/
	WriteMemory(pfGetDesk,newCodeG,CODE_LENGTH);
}
...
void HookOff()
{
	...
	/*aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa*/
	WriteMemory(pfGetDesk,oldCodeG,CODE_LENGTH);
}
...
void Inject()
{
	...
	/*aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa*/
	oldGetDesk = (TypeGetDesk) ::GetProcAddress(hmodle,"GetDesktopWindow");
	pfGetDesk = (FARPROC)oldGetDesk;
	...
	if ( pfGetDesk == NULL)
	{
		AfxMessageBox(_T("获取 GetDesktopWindow 函数失败"));
		return;
	}
	...
	_asm
	{
		lea edi,oldCodeG	// 取数组基地址
		mov esi,pfGetDesk	// API地址
		cld					// 设置方向
		mov ecx,CODE_LENGTH
		rep movsb
	}
	...
	/*aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa*/
	newCodeG[0] = 0xe9;   // jmp 指定代码
	_asm
	{
		lea eax,MyGetDesktopWindow		// 新API地址
		mov ebx,pfGetDesk			// 原API地址
		sub eax,ebx				
		sub eax,CODE_LENGTH			// 跳转地址 = 新API地址 - 原API地址 - 指令长度
		mov dword ptr [newCodeG+1],eax // eax 32bit =  4 BYTE
	}
...
/*	
	...
	HWND MyGetDesktopWindow(void)
{
	HWND hWnd,LPCWSTR lpText,LPCWSTR lpCation,UINT uType;
	int nRet = 0;
	HWND res;
...
/*
	...
	info.Format(_T("进程名 %s"),procName);
	AfxMessageBox(info);*/
	nRet = ::MessageBoxA(hWnd,"哈哈 ^_^，GetDesktopWindow 被 HOOK 咯",lpCation,uType);
