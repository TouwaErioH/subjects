...
def signup(user_message: dict):
    ...
    resign=False
    try:
        conn = pymysql.connect(CONFIG['host'], CONFIG['user'], CONFIG['pwd'], CONFIG['db'])
        cursor = conn.cursor()
        cursor.execute('''
            SELECT *
            FROM student
...
# 搜索书籍
def search_book(info: str, searchrestrict: str, SID: str = '') -> list:
    '''
    ...
    try:
        ...
        if info == 'ID/书名/作者/出版社' or info == '':
        ...
        elif searchrestrict != 'BID' and searchrestrict != 'CLASSIFICATION':
            # AUTHOR或PRESS或BNAME
            if searchrestrict=='PRESS':
                cursor.execute('''
                            SELECT *
                            FROM book
                            WHERE PRESS LIKE %s
                            ''', ('%' + info + '%'))
            elif searchrestrict=='BNAME':
                cursor.execute('''
                            SELECT *
                            FROM book
                            WHERE  BNAME LIKE %s
            ...
            ''', (info))
            res = tuple_to_list(cursor.fetchall())
        elif searchrestrict == 'CLASSIFICATION':
            # 通过分类搜书
            cursor.execute('''
            SELECT BID
            FROM classification
